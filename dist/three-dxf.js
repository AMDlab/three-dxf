...
import WorkerWrapper from 'three-dxf/dist/worker-wraper';
import CreateObjectWorkerJs from 'raw-loader!three-dxf/dist/create-object.worker';
...
export async function Viewer(data, parent, opt = DEFAULT_OPT, cEvents = {}) {
  ...
  const reformedLines = reformLines(result.lineVecs)
  const intersects = createIntersectPoint(reformedLines)
...
async function createObjects(data, font) {
  ...
  const worker = new WorkerWrapper(URL.createObjectURL(new Blob([CreateObjectWorkerJs])));
  const promises = []
  ...
  for (const value of values) {
    const obj = value.mesh
    if (!obj) continue;
    if (obj.object.type == 'Line') {
      objs.push(Object.assign(new THREE.Line, obj))
    } else {
      objs.push(Object.assign(new THREE.Object3D, obj))
    }
    lineVecs = lineVecs.concat(value.lineVecs)
  }
 ...
 * @return Array
 */
function createIntersectPoint(lines) {
  let intersects = []

  for(const line of lines) {
    const lineVec = line.lineVec
    const finieshLines = lines.slice(0, line.index)
    intersects = intersects.concat(findCrossPoint(lineVec.start, lineVec.end, finieshLines))
  }
  return intersects
}
